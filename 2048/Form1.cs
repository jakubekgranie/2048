...
namespace _2048
{
    public partial class Form1 : Form
    {
        ...
        bool first = true, unlocked = true;
        private void Form1_KeyUp(object sender, KeyEventArgs e)
        {
            if ((e.KeyCode == Keys.W || e.KeyCode == Keys.A || e.KeyCode == Keys.S || e.KeyCode == Keys.D) && unlocked)
            {
                        ...
                        for (int j = 0; j < 4; j++)
                            if (assets[i, j].Text == " ")
                            {
                                ...
                                do
                                {
                                    x = rnd.Next(0, 4);
                                    y = rnd.Next(0, 4);
                                } while (assets[x, y].Text != " ");
                                int chance = rnd.Next(1, 101);
                                if (chance < 71)
                                {
                                    assets[x, y].Text = "2";
                                    tableLayoutPanel1.GetControlFromPosition(y, x).BackColor = Color.FromArgb(255, colors[0, 0], colors[0, 1], colors[0, 2]);
                                }
                                else if (chance < 86)
                                {
                                    assets[x, y].Text = "4";
                                    tableLayoutPanel1.GetControlFromPosition(y, x).BackColor = Color.FromArgb(255, colors[1, 0], colors[1, 1], colors[1, 2]);
                                }
                                else if (chance < 96)
                                {
                                    assets[x, y].Text = "8";
                                    tableLayoutPanel1.GetControlFromPosition(y, x).BackColor = Color.FromArgb(255, colors[2, 0], colors[2, 1], colors[2, 2]);
                                }
                                else if (chance < 101)
                                {
                                    assets[x, y].Text = "16";
                                    tableLayoutPanel1.GetControlFromPosition(y, x).BackColor = Color.FromArgb(255, colors[3, 0], colors[3, 1], colors[3, 2]);
                                }
                                return;
                            }
                }
                ...
                async void checkWin()
                {
                    if (assets[3, 3].Text != assets[3, 2].Text && assets[3, 3].Text != assets[2, 3].Text && assets[3, 3].Text != " ")
                    {
                        for (int i = 0; i < 4; i++)
                            for (int j = 0; j < 4; j++)
                                if ((i + 1 < 4 && assets[i, j].Text == assets[i + 1, j].Text) || (j + 1 < 4 && assets[i, j].Text == assets[i, j + 1].Text) || assets[i, j].Text == " ")
                                    return;
                    }
                    else return;
                    unlocked = false;
